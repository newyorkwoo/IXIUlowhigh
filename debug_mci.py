...
for i in range(1, len(df)):
    ...
    if drop_days > 0 and drop_pct < 0:
        ...
        vix_max = df['VIX'].iloc[c_peak_idx:i+1].max()
        fear_premium = (vix_max + max(0, vix_max - c_vix_start)) / 20.0
        rsi_min = df['RSI60'].iloc[c_peak_idx:i+1].min()
        oversold = (50.0 / max(1, rsi_min)) ** 2
        df.at[df.index[i], 'Daily_MCI'] = min(100.0, price_damage * fear_premium * oversold)

mask = (df['DateStr'] >= '2024-07-11') & (df['DateStr'] <= '2024-09-15')
